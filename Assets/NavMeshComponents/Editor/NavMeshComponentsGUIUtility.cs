...
namespace UnityEditor.AI
{
    public static class NavMeshComponentsGUIUtility
    {
        ...
        public static void AgentTypePopup(Rect position, string labelName, SerializedProperty agentTypeID)
        {
            var index = -1;
            var count = NavMesh.GetSettingsCount();
            var agentTypeNames = new string[count + 2];
            ...
            if (!validAgentType)
            {
                EditorGUI.HelpBox(position, "Agent Type invalid.", MessageType.Warning);
            }
